...
package io.pivotal.spring.cloud.stream.gemfire.embedded.common;
...
public class GemfirePoolConfiguration {
	...
	public PoolFactoryBean gemfirePool() {
		...
		switch (config.getConnectType()) {
			case locator:
				poolFactoryBean.setLocators(Arrays.asList(this.config.getHostAddresses()));
				break;
			case server:
				poolFactoryBean.setServers(Arrays.asList(this.config.getHostAddresses()));
				break;
			default:
